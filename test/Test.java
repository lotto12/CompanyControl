        ...
        for (i = 0; i < data_table.getRow(); i++) {
            ...
            switch (type) {
                case "1":
                    ...
                    //讀取所有階層設定
                    sq_sel = "SELECT * FROM game_agents_conf WHERE id IN (" + ag_id + "," + sa_id + "," + co_id + "," + sc_id + "," + mm_id + "," + bm_id + ") ORDER BY member ";
                    temp = DB.getDataTable(sq_sel);
                    ...
                    DB.query(sq_up);
                    //寫入資料
                    sq_in = "INSERT INTO game_cal_801_" + type + " (game_id,wagers_id,gtype,status,num,mem_id,ag_id,sa_id,co_id,sc_id,mm_id,bm_id,"

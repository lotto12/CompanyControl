...
/**
 *
 * @author JimmyYang 網頁路徑GamePage/game1/Right.jsp
 */
public class GamePage_game1_Right1 {
    ...
    public String getHTML_TABLE(String game_id, String gtype) {
        ...
        for (int i = 0; i < table_limit; i++) {
            ...
            for (int j = 0; j < count; j++) {
                ...
                String total_cash = getBet_gold(game_id, gtype, "1", data_count); //下注總金額
    ...
    private String getTotal_Sum(String game_id, String gtype) {
        String sql_select = "select Sum(bet_gold) as total from game_warges_main where id in (select flw_id from game_warges_sub where gtype = " + gtype + " and type=1) and game_id = '" + game_id + "'";
        String result = "0";
        String[][] data = DB.getDataArray(sql_select);
        if (data.length > 0) {
            result = data[0][0];
